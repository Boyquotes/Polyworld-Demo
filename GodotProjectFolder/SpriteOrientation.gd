...
func _physics_process(_delta):
	...
	var rotating_direction_scalar = roundi(float(is_rotating_clockwise) * -2 + 1)
	rotating_direction_scalar = -1
	
	var dir = get_parent().global_transform.basis.z
	var dir_2d = Vector2(dir.x * rotating_direction_scalar, dir.z)
