...
@onready var cam_gimbal = get_viewport().get_camera_3d().get_parent() as CameraArm
...
func _physics_process(delta):
	...
	#ai_move(delta, player.global_transform.origin, 2.0, false, true, false, 2.0)
	if is_on_floor():
		ai_move(delta, player.global_position, 3.0)
